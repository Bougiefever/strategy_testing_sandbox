...
Here are some ETFs for testing along with the unlevered proxy for the 20-day return filter

Use the unlevered proxy for the 20-day return filter:
...
params = {
    ...
    'breakeven_trigger_multiplier': 0.01,
    'starting_equity': 100_000,
    'equity_pct': 0.25,
    'run_number': 1
}

daily_path = Path(r'D:\stock_data\daily')
base_df_fn = daily_path.joinpath(f'{params["base_ticker"]}_.parquet')
df_base = pd.read_parquet(base_df_fn, engine='pyarrow')
# df_base['200ma'] = talib.SMA(df_base['close'].to_numpy(float), timeperiod=200)
# df_base['yesterday_200_ma'] = df_base['200ma'].shift(1)
df_base['return20d'] = df_base['close'].pct_change(20)
df_base['yesterday_return20d'] = df_base['return20d'].shift(1)
df_base.set_index('quote_datetime', inplace=True)

output_folder = Path(r'D:\test_data\stock_commodities\mean_rev_weekly_trade')
output_folder.mkdir(parents=True, exist_ok=True)
etf_folder = Path(r'D:\stock_data\intraday\stocks\etfs')
fn = etf_folder.joinpath(f'{params['ticker']}.parquet')
...
df = df.loc[params['start_date']:params['end_date']]
...
def get_exit(profit_target_dt, breakeven_trigger_dt, stop_dt,
    ...
    return friday_close_dt, friday_close_price, 'TIME_CLOSE'
...
for dt in monday_dts:
    ...
    if today not in df_base.index:
        continue
    twenty_day_return = df_base.loc[today, 'yesterday_return20d']
    ...
    if not pd.isna(breakeven_trigger_dt):
        start_dt = breakeven_trigger_dt - pd.Timedelta(minutes=1)
        wednesday = 2
        df_last_day = df_week[df_week.index.weekday == wednesday]
        if df_last_day.empty:
            df_last_day = df_week[df_week.index.weekday < wednesday]

        end_dt = df_last_day.iloc[-1].name
        #     break
        #end_dt = df_week.iloc[-1].name
        stop_scan_window = df_week.loc[start_dt:end_dt]   #[df_week.index > breakeven_trigger_dt]
        stop_scan = stop_scan_window['high'] >= entry_px
        stop_dt = first_true_ts(stop_scan)
        end_close_px = df_week.loc[end_dt, 'close']
    else:
        stop_dt = pd.NaT
        end_dt = df_week.iloc[-1].name
    end_close_px = df_week.iloc[-1]['close']

    exit_dt, exit_px, exit_reason = get_exit(profit_target_dt, breakeven_trigger_dt, stop_dt,
                                                profit_target_px, entry_px, end_close_px, end_dt)
...
params_fn = output_folder.joinpath(f'{params["ticker"]}_params_{params["run_number"]}.csv')
params_series.to_csv(params_fn, index=True)
stats_fn = output_folder.joinpath(f'{params["ticker"]}_trade_stats_{params["run_number"]}.csv')
df_stats.to_csv(stats_fn, index=True)
trades_fn = output_folder.joinpath(f"{params["ticker"]}_trades_{params["run_number"]}.csv")
df_trades.to_csv(trades_fn, index=False)
port_stats_fn = output_folder.joinpath(f'{params["ticker"]}_portfolio_stats_{params["run_number"]}.csv')
