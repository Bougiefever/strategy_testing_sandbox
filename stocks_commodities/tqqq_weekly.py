...
for dt in monday_dts:
    ...
    entry_px = monday_px - (monday_px * 0.01)

    # price must drop by 1% to trigger a trade
    entry_scan = df_week['low'] <= entry_px
    ...
    breakeven_trigger_px = entry_px - (entry_px * 0.005)
    ...
    shares = max(100, int(np.floor(equity*0.5 / entry_px)))
    ...
    # # get trade value at every hour
    # trade_window = df_week[entry_dt:exit_dt].reset_index()
    # hourly = trade_window.groupby(pd.Grouper(key='quote_datetime', freq='h')).first().reset_index()
...
trades_fn = output_folder.joinpath("trades_2022.csv")
...
# df_history = pd.concat(trade_datas)
# history_fn = output_folder.joinpath("history_2022.csv")
# df_history.to_csv(history_fn, index=False)
