...
output_folder = Path(r'D:\test_data\petrou')
...
start_date = df_lev.index[0] #datetime.datetime(2010, 4, 9)
end_date = df_lev.index[-1]  #datetime.datetime(2026, 12, 31)
...
if __name__ == '__main__':
    ...
    for dt in dts:
        ...
        if current_trade is not None:
            ...
            trailing_stop = peak_high * (1 - trailing_stop_pct) * (1 - trailing_buffer)
            active_stop = max(active_stop, hard_stop, trailing_stop)
            ...
            if close < active_stop:
                exit_trade = True
                stopped_out = True if macd > 0 else False
                exit_reason = "active_stop"
            elif macd < 0:
                exit_trade = True
                exit_reason = "macd lt 0"
            if exit_trade:
        ...
        # Check for trade entry - but not on the same day as exit
            ...
            shares = int(np.floor((equity) / entry_px))
            enter_trade = False
    ...
    portfolio_stats, trade_stats = print_report(ticker, df_trades, df_daily,
                                                f"{leveraged_ticker} Weekly MACD Strategy", frequency='weekly')
